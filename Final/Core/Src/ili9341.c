...
#include "main.h" // Nếu bạn đã khai báo trong main.h
void LCD_BL_ON(void) {
...
const uint8_t Font8x8[] = {
    ...
    //...
    // Bạn cần khai báo tiếp các ký tự ASCII tiếp theo, có thể tham khảo từ các thư viện bitmap font.
    // Mỗi ký tự là một mảng 8 byte (8 dòng, mỗi dòng 8 bit) biểu diễn pixel.
};
...
void LCD_DrawChar(uint16_t x, uint16_t y, char c, uint16_t color, uint16_t bgcolor) {
    ...
    uint8_t line;
    for (int i = 0; i < 8; i++) {
        line = Font8x8[(c - 32) * 8 + i];
        for (int j = 0; j < 8; j++) {
            if (line & (1 << (7 - j))) {
                lcdDrawPixel(x + j, y + i, color);
            } else {
                lcdDrawPixel(x + j, y + i, bgcolor);
            }
        }
    }
}
...
void LCD_Print(uint16_t x, uint16_t y, char* str, uint16_t color, uint16_t bgcolor) {
    ...
    while (*str) {
        ...
        xpos += 8;  // giả sử font rộng 8 pixel
        str++;
    }
}
...
lcdPropertiesTypeDef lcdProperties = {
    ILI9341_PIXEL_WIDTH,
    ILI9341_PIXEL_HEIGHT,
